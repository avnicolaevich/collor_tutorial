...
;
(function (a) {
    var m, t, u, f, D, h, E, n, z, A, q = 0, e = {}, o = [], p = 0, c = {}, l = [], I = null, v = new Image, J = /\.(jpg|gif|png|bmp|jpeg)(.*)?$/i, W = /[^\.]\.(swf)\s*$/i, K, L = 1, y = 0, s = "", r, j, i = false, B = a.extend(a("<div/>")[0], {prop: 0}), M = a.browser.msie && a.browser.version < 7 && !window.XMLHttpRequest, N = function () {
            t.hide();
                ...
                if (c.titleShow !== false) {
                    ...
                    if (!(!s || s === "")) {
                        ...
                        switch (c.titlePosition) {
                            case "inside":
                                n.css({width: j.width - c.padding * 2, marginLeft: c.padding, marginRight: c.padding});
                ...
                if (f.is(":visible")) {
                    ...
                    h.fadeTo(c.changeFade, 0.3, function () {
                        ...
                        if (d)g(); else {
                            B.prop = 0;
                            a(B).animate({prop: 1},
                                {duration: c.changeSpeed, easing: c.easingChange, step: S, complete: g})
                        }
                    })
                } else {
                    f.removeAttr("style");
                    ...
                    if (c.transitionIn == "elastic") {
                        ...
                        B.prop = 0;
                        a(B).animate({prop: 1}, {duration: c.speedIn, easing: c.easingIn, step: S, complete: R})
                    } else {
        ...
        S = function (b) {
            ...
            if (b && b.length) {
                ...
                d.height = b.height();
                d = {
                    width: d.width + c.padding * 2, height: d.height +
                    c.padding * 2, top: d.top - c.padding - 20, left: d.left - c.padding - 20
                }
            } else {
                b = T();
                d = {
        ...
        if (!i) {
            ...
            if (a.isArray(b)) {
                ...
                if (typeof b == "object")a(b).data("fancybox",
                    a.extend({}, g, b)); else b = a({}).data("fancybox", a.extend({content: b}, g));
                o.push(b)
            }
    ...
    a.fancybox.close = function () {
        ...
        if (!(i || f.is(":hidden"))) {
            i = true;
            if (c && false === c.onCleanup(l, p, c))i = false; else {
                N();
                a(E.add(z).add(A)).hide();
                a(h.add(u)).unbind();
                a(window).unbind("resize.fb scroll.fb");
                ...
                if (c.transitionOut == "elastic") {
                    r = V();
                    var d = f.position();
                    j = {top: d.top, left: d.left, width: f.width(), height: f.height()};
                    if (c.opacity)j.opacity = 1;
                    n.empty().hide();
                    B.prop = 1;
                    a(B).animate({prop: 0}, {duration: c.speedOut, easing: c.easingOut, step: S, complete: b})
